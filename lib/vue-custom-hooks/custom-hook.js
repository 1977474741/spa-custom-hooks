    ...
    init() {
        ...
        Object.keys(this.pageHooks).filter(e => {
            ...
            hookInscape[e].forEach(hookStr => {
                const item = hook[hookStr];
                if (item && item.init) {
                    this.hooksStr[index].name.push(hookStr);
                    if(item.destroy && item.name){
                        // 不支持的钩子不予处理
                        let pageHooks = this.pageHooks;
                        // 钩子和对立钩子回调数组
                        const hookArr = pageHooks[item.name] || [],
                              contraryHookArr = pageHooks[item.destroy] || [];
                        if(hookArr.length && contraryHookArr.length){
                            item.need = true;
                        };
                    }else{
                        item.need = true;
